...
public class OracleDialect implements Dialect {
	...
	public static void main(String[] args){
		String sql = "  Select a. id , b  .Name UserName,    c .   password As PASSWORD FROM aa a,   bb b,  cc c";
		OracleDialect oracleDialect = new OracleDialect();
		System.out.println(oracleDialect.getPageCountString(sql));
		System.out.println(oracleDialect.getPageDataString(sql, 1, 10));
		String[] str = new OracleDialect().getQueryStringArrayWithResultMap(sql);
		for (int i = 0; i < str.length; i++) {
			System.out.println(str[i]);
		}
//		String fieldsString = sql.replaceAll("\r","").replaceAll("\n","").replaceFirst(pageNavigationExtRegEx,"$2").replaceAll("\\w+\\s*\\.","").replaceAll("(?i)as","");
//		//去掉回车，去掉换行，取字段部分，去掉别名和点，去掉as，成了最简单的字段部分
//		String[] fields = fieldsString.split(",");
//		for(int i = 0; i < fields.length;i++){
//			String[] tempFieldArray = fields[i].trim().split("(\\s+)");
